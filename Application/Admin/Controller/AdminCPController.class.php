...
class AdminCPController extends AdminController{
    ...
    public function index(){
    	...
    	$menu_list = D('admin_menu')->where($map)->order('listorder,id')->select();
    	//菜单按钮多语言
    	foreach ($menu_list as $key => $menu){
    		$language = cookie('think_language');
    		if($menu['name_'.$language]){
    			$menu_list[$key]['name'] = $menu['name_'.$language];
    		}
    	}
    		
    	$menu=list_to_tree($menu_list,'id','parentid','_child');
